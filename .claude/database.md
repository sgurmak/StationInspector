...
| `poiItemJson` | String? | Serialized PoiItem JSON |
...
- **`StationWithPhotoCount`** — `@Embedded StationEntity` + `photoCount: Int`
- **`StationWithSplitCounts`** — `@Embedded StationEntity` + `regularCount: Int` (CLIENT_REPORT) + `issueCount: Int` (INTERNAL_DEFECT)
...
| 7→8 | Add `orderIndex` to `stations`; drop and recreate `pois` with new schema (id as String PK, added city/address/region/inspectionDate/orderIndex) |

**Safety net**: `fallbackToDestructiveMigration()` is enabled — if a migration path is missing, the DB is wiped.

## DAOs

- **StationDao**: CRUD + getStationsWithSplitCounts (JOIN query), updateStationOrders (transaction)
- **PhotoDao**: CRUD + filter by station, by station+zone, unexported
- **RouteCacheDao**: insert + lookup by ID or coordinates
- **PoiDao**: insert + getPoisForDate + updatePoiOrders (transaction) + delete
